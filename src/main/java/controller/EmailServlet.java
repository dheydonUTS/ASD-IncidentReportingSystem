...
public class EmailServlet extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        //Define MailSSLSocketFactory, use for SSL
        try {
        ...
        //Determine the message type (currently only warning)
        if ("warning".equals(request.getAttribute("emailType"))) {
            Warning warning = (Warning) request.getAttribute("Warning");
            //Generate the email contents using defined properties and warning
            Message message = generateWarningMessage(session, warning);
            try {
                //Then send the email we generated
                Transport.send(message);
            } catch (MessagingException ex) {
                //If we fail to send the message, redirect user
                Logger.getLogger(EmailServlet.class.getName()).log(Level.SEVERE, null, ex);
                request.getRequestDispatcher("error.jsp").include(request, response);

            }
        };
        //If we get to here we have been successful sending the email, tell the user
    ...
    //Function that generates the minified email using provided warning, offender and venue details
